#!/usr/bin/env bash
# Reproducible preprocessing for GOLD (CCDC) (tool/gold).
...
#
# If GOLD (CCDC) is GUI/web-only and cannot be scripted, leave this as documentation and
# record the exact settings/clicks in steps.md instead.
set -euo pipefail

HERE="$(cd "$(dirname "$0")" && pwd)"
TOOL_DIR="$(dirname "$HERE")"                 # tools/gold
...
echo "[gold] receptor : $RECEPTOR"
echo "[gold] ligands  : $LIGANDS"
echo "[gold] box      : $BOX"
...
echo "[gold] preprocessing done. Now fill PREPROCESSING.yaml and run ../docking/run_docking.sh"
