#!/usr/bin/env bash
# Reproducible preprocessing for {{TOOL_NAME}} ({{BRANCH}}).
...
#
# If {{TOOL_NAME}} is GUI/web-only and cannot be scripted, leave this as documentation and
...
echo "[{{SLUG}}] receptor : $RECEPTOR"
echo "[{{SLUG}}] ligands  : $LIGANDS"
echo "[{{SLUG}}] box      : $BOX"
...
echo "[{{SLUG}}] preprocessing done. Now fill PREPROCESSING.yaml and run ../docking/run_docking.sh"
